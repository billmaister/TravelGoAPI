...
const getRecommendedCountries = async (req, res) => {
  try {
    const { currentRecommendations } = await Recommendation.findOne({
      type: "countries",
    });
    const recommendations = await Country.find({
      _id: { $in: currentRecommendations },
    });
    res.status(200).json({
      success: true,
...
const getFavouriteCountries = async (req, res) => {
  try {
    ...
    const { favouriteCountries } = await User.findOne(
      { _id: ObjectId(userID) },
      { favouriteCountries: 1 }
    );
    const completeCountryInformation = await Country.find({
      _id: { $in: favouriteCountries },
    });
    res.status(200).json({
      success: "success",
